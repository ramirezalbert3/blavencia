...
public:
    ...
    {
        empty() {shape_.setFillColor(sf::Color::Black);}
        std::unique_ptr<cell_impl_t> copy() const override {return std::make_unique<empty>(*this);};
    };

    struct wall : public cell_impl_t
    {
        wall() {shape_.setFillColor(sf::Color::Green);}
        std::unique_ptr<cell_impl_t> copy() const override {return std::make_unique<wall>(*this);};
    };
